...
public class TowerTypeBuilder {
    ...
    private List<TowerType> getData(float scale) {
        ...
        // Shop
        specs.add(Specs.getSpecs(TowerName.Shop, 0, null, 18, 0, null, Treasure.fromNone()));
        /// ######################  PURES
        // Basic
        specs.add(new Specs(TowerName.Needle, 1, Normal, N_SPEED, N_RANGE, 18,1, HitAbility.getNormal("bullet", scale), Treasure.fromNone().addGold(7)));
        specs.add(new Specs(TowerName.Pinch, 2, Normal, N_SPEED, N_RANGE, 54,1, HitAbility.getNormal("bullet", scale), Treasure.fromNone().addGold(13)));
        specs.add(new Specs(TowerName.Sting, 3, Normal, N_SPEED, N_RANGE, 162,1, HitAbility.getNormal("bullet", scale), Treasure.fromNone().addGold(37)));
        specs.add(new Specs(TowerName.Scratch, 1, Splash, S_SPEED, N_RANGE, 5,1, HitAbility.getSplash("bullet", scale, 2), Treasure.fromNone().addGold(7)));
        specs.add(new Specs(TowerName.Bite, 2, Splash, S_SPEED, N_RANGE, 14,1, HitAbility.getSplash("bullet", scale, 2), Treasure.fromNone().addGold(13)));
        specs.add(new Specs(TowerName.Smash, 3, Splash, S_SPEED, N_RANGE, 41,1, HitAbility.getSplash("bullet", scale, 2), Treasure.fromNone().addGold(37)));
        // blue
